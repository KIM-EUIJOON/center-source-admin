...
using AppSigmaAdmin.Repository.Ainokaze;
using AppSigmaAdmin.Repository.Toyamachitetsu;

namespace AppSigmaAdmin.Controllers
{
    public class MPA1602Controller : Controller
    {    
        //ファイル出力関連
        ...
        // セッションキー
        private const string SESSION_SEARCH_MPA1602 = "SESSION_SEARCH_MPA1602";

        // 一覧ページ単位の表示件数
        private const int LISTMAXROW = 10;

        // ルートパス
        private const string CONTENTROOT = "~/Views/MPA1602";

        /// <summary>
        /// 決済データ画面(初期, ページ遷移)
        /// </summary>
        /// <returns>ログイン画面</returns>
        [SessionCheck(WindowName = "乗車券利用実績画面(富山地方鉄道様)")]
        public ActionResult Index(string page)
        {
            ...
            if (string.IsNullOrEmpty(page))
            {
                ...
                return View(new MPA1602Context());
            }

            //セッション情報の取得
            Dictionary<string, string> searchKey = new Dictionary<string, string>();
            searchKey = (Dictionary<string, string>)Session[SESSION_SEARCH_MPA1602];
            //検索条件：開始日時
            ...
            if (usageList.Any())
            {
                info.UsageReportData = new MPA1602ReportData()
                {
                    ListMaxCount = int.Parse(maxListCount),
                    ListNum = ListNum,
                    ReportList = usageList.Select(ToMPA1602ReportDataRow()).ToList(),
                };
            }
            else
            {
        ...
        [HttpPost]
        [SessionCheck(WindowName = "乗車券利用実績画面(富山地方鉄道様)")]
        public ActionResult Index(MPA1602Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                Session.Add(SESSION_SEARCH_MPA1602, searchKey);
        ...
        [HttpPost]
        [SessionCheck(WindowName = "乗車券利用実績画面(富山地方鉄道様)")]
        public ActionResult OutPutCSV(MPA1602Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                using (var sw = new StreamWriter(ms, System.Text.Encoding.GetEncoding("shift_jis")))
                {
                    ...
                    //ファイル名を「Toyamachitetsu_Usage_検索開始日(yyyyMMdd)-終了日(yyyyMMdd)_作成日(yyyyMMdd)」で出力
                    return File(ms.ToArray(), FILE_CONTENTTYPE, "Toyamachitetsu_Usage_" + targetDateStart.ToString("yyyyMMdd") + "-" + targetDateLast.ToString("yyyyMMdd") + "_" + NowDate.ToString("yyyyMMdd") + FILE_EXTENSION);
                }
            }
            // 入力エラー
            catch(InvalidInputException iie)
        ...
        /// </summary>
        /// <returns></returns>
        private static ToyamachitetsuTicketSaleRepository GetTicketListRespository()
        {
            return new ToyamachitetsuTicketSaleRepository();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private static ToyamachitetsuTicketUsageRepository GetTicketUsageRepository()
        {
            return new ToyamachitetsuTicketUsageRepository();
        }
        ...
        private static void IsValidInput(MPA1602Context model)
        {
        ...
        private static (int Begin, int End) GetListRowNum(MPA1602Context model)
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private static string GetSelectedTicket(MPA1602Context model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static Func<ToyamachitetsuUsageInsuranceInfo, MPA1602ReportDataRow> ToMPA1602ReportDataRow() =>
            t =>
                new MPA1602ReportDataRow()
                {
                    UserId = t.UserId,
        ...
        /// <returns></returns>
        private static List<SelectListItem> CreatePDLItemsTicketList(IEnumerable<ToyamachitetsuPaymentInfo> infos, string selected = null)
