...
{
    public class MPA0601Controller : Controller
    {
        ...
        private const string SESSION_SEARCH_JRKyushuPayment = "SESSION_SEARCH_JRKyushuPayment";
        ...
        [SessionCheck(WindowName = "JR九州決済画面")]
        public ActionResult Index(string page)
        {
            ...
            //プルダウンリスト
            List<NishitetsuPaymentInfo> JRKyushuTicket = null;
            JRKyushuTicket = new JRKyushuPaymentModel().JRKyushuPassportList(UserRole);
            ...
            {
                foreach (var TicketList in JRKyushuTicket)
                {
            ...
            searchKey = (Dictionary<string, string>)Session[SESSION_SEARCH_JRKyushuPayment];
            //検索条件：開始日時
            ...
            //取得したリスト件数が0以上
            if (SelectJRKyushuPaymentDateList.Count > 0)
            {
                info.NishitetsuPaymentInfoList = SelectJRKyushuPaymentDateList;
            }
            else
            {
        ...
        public ActionResult Index(NishitetsuPaymentInfoListEntity model)
        {
            ...
            //西鉄決済データ最大件数取得用リスト
            List<NishitetsuPaymentInfo> JRKyushuPaymentDateListMaxCount = null;
            //西鉄決済データ表示件数分取得用リスト
            List<NishitetsuPaymentInfo> SelectJRKyushuPaymentDateList = null;
            ...
            Session.Add(SESSION_SEARCH_JRKyushuPayment, searchKey);
        ...
        public ActionResult JRKyushuOutPutDate(NishitetsuPaymentInfoListEntity model)
        {
            ...
            if (string.IsNullOrEmpty(model.TargetDateBegin))
            {
                //検索期間(開始)が未入力の場合
                ModelState.AddModelError("", "表示期間の開始年月日を指定してください");
                return View("~/Views/MPA0601/Index.cshtml", model);
            }
            else if (string.IsNullOrEmpty(model.TargetDateEnd))
            {
                //検索期間(終了)が未入力の場合
            ...
            {
                ModelState.AddModelError("", "一致する決済データがありませんでした。");
                return View("~/Views/MPA0601/Index.cshtml", info);
            }


            MemoryStream JRKyushuMem = new MemoryStream();
            StreamWriter JRKyushusw = new StreamWriter(JRKyushuMem, System.Text.Encoding.GetEncoding("shift_jis"));
            //ヘッダー部分を書き込む
            JRKyushusw.Write("\"myroute会員ID\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"決済日時\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"決済ID\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"カテゴリー\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"商品種別\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"大人枚数\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"子供枚数\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"決済種別\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"仕向先\"");
            JRKyushusw.Write(',');
            JRKyushusw.Write("\"金額\"");
            JRKyushusw.Write(',');
            ...
            {
                //文字列に"を追加して出力
                JRKyushusw.Write("\"" + item.UserId.ToString() + "\"");        //myrouteID
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.TranDatetime.ToString() + "\"");  //決済日時
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.PaymentId.ToString() + "\"");     //決済ID
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.TransportType.ToString() + "\"");    //チケット種別
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.TicketName.ToString() + "\"");    //商品種別
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.AdultNum.ToString() + "\"");      //大人枚数
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.ChildNum.ToString() + "\"");      //子供枚数
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.PaymentType.ToString() + "\"");   //決済種別
                JRKyushusw.Write(',');
                JRKyushusw.Write("\"" + item.GetForwardName() + "\"");   //仕向先
                JRKyushusw.Write(',');
            ...
            return File(JRKyushuMem.ToArray(), FILE_CONTENTTYPE, "JRKyushu_Report_" + TargetDateStart.ToString("yyyyMMdd") + "-" + TargetDateLast.ToString("yyyyMMdd") + "_" + NowDate.ToString("yyyyMMdd") + FILE_EXTENSION);
        }
    }
}