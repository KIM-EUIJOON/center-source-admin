...
{
    public class MPA1601Controller : Controller
    {
        //ファイル出力関連
        ...
        // セッションキー
        private const string SESSION_SEARCH_MPA1601 = "SESSION_SEARCH_MPA1601";

        // 一覧ページ単位の表示件数
        private const int LISTMAXROW = 10;

        // ルートパス
        private const string CONTENTROOT = "~/Views/MPA1601";
        ...
        [SessionCheck(WindowName = "富山地方鉄道決済画面")]
        public ActionResult Index(string page)
        {
            ...
            if (string.IsNullOrEmpty(page))
            {
                ...
                return View(new MPA1601Context());
            }

            //セッション情報の取得
            Dictionary<string, string> searchKey = new Dictionary<string, string>();
            searchKey = (Dictionary<string, string>)Session[SESSION_SEARCH_MPA1601];
            //検索条件：開始日時
            ...
            if (paymentList.Any())
            {
                info.PaymentReportData = new MPA1601ReportData()
                {
                    ListMaxCount = int.Parse(maxListCount),
                    ListNum = ListNum,
                    ReportList = paymentList.Select(ToMPA1601ReportDataRow()).ToList(),
                };
            }
            else
            {
        ...
        [HttpPost]
        [SessionCheck(WindowName = "富山地方鉄道決済データ画面")]
        public ActionResult Index(MPA1601Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                Session.Add(SESSION_SEARCH_MPA1601, searchKey);
        ...
        [HttpPost]
        [SessionCheck(WindowName = "富山地方鉄道決済データ画面")]
        public ActionResult OutPutCSV(MPA1601Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                using (var sw = new StreamWriter(ms, System.Text.Encoding.GetEncoding("shift_jis")))
                {
                    ...
                    //ファイル名を「Toyamachitetsu_Report_検索開始日(yyyyMMdd)-終了日(yyyyMMdd)_作成日(yyyyMMdd)」で出力
                    return File(ms.ToArray(), FILE_CONTENTTYPE, "Toyamachitetsu_Report_" + targetDateStart.ToString("yyyyMMdd") + "-" + targetDateLast.ToString("yyyyMMdd") + "_" + NowDate.ToString("yyyyMMdd") + FILE_EXTENSION);
                }
            }
            // 入力エラー
            catch (InvalidInputException iie)
        ...
        /// </summary>
        /// <returns></returns>
        private static ToyamachitetsuTicketSaleRepository GetTicketListRepository()
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static ToyamachitetsuTicketPaymentRepository GetTicketPaymentRepository()
        {
            return new ToyamachitetsuTicketPaymentRepository();
        }
        ...
        private static void IsValidInput(MPA1601Context model)
        {
        ...
        private static (int Begin, int End) GetListRowNum(MPA1601Context model)
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private static string GetSelectedTicket(MPA1601Context model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static Func<ToyamachitetsuPaymentInfo, MPA1601ReportDataRow> ToMPA1601ReportDataRow() =>
            t =>
                new MPA1601ReportDataRow()
                {
                    UserId = t.UserId,
