...
{
    public class MPA1501Controller : Controller
    {
        //ファイル出力関連
        ...
        // セッションキー
        private const string SESSION_SEARCH_MPA1501 = "SESSION_SEARCH_MPA1501";

        // 一覧ページ単位の表示件数
        private const int LISTMAXROW = 10;

        // ルートパス
        private const string CONTENTROOT = "~/Views/MPA1501";
        ...
        [SessionCheck(WindowName = "あいの風とやま決済画面")]
        public ActionResult Index(string page)
        {
            ...
            if (string.IsNullOrEmpty(page))
            {
                ...
                return View(new MPA1501Context());
            }

            //セッション情報の取得
            Dictionary<string, string> searchKey = new Dictionary<string, string>();
            searchKey = (Dictionary<string, string>)Session[SESSION_SEARCH_MPA1501];
            //検索条件：開始日時
            ...
            if (paymentList.Any())
            {
                info.PaymentReportData = new MPA1501ReportData()
                {
                    ListMaxCount = int.Parse(maxListCount),
                    ListNum = ListNum,
                    ReportList = paymentList.Select(ToMPA1501ReportDataRow()).ToList(),
                };
            }
            else
            {
        ...
        [HttpPost]
        [SessionCheck(WindowName = "あいの風とやま決済データ画面")]
        public ActionResult Index(MPA1501Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                Session.Add(SESSION_SEARCH_MPA1501, searchKey);
        ...
        [HttpPost]
        [SessionCheck(WindowName = "あいの風とやま決済データ画面")]
        public ActionResult OutPutCSV(MPA1501Context model)
        {
            try
            {
                ViewData["message"] = "";
        ...
        /// </summary>
        /// <returns></returns>
        private static AinokazeTicketSaleRepository GetTicketListRepository()
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static AinokazeTicketPaymentRepository GetTicketPaymentRepository()
        {
            return new AinokazeTicketPaymentRepository();
        }
        ...
        private static void IsValidInput(MPA1501Context model)
        {
        ...
        private static (int Begin, int End) GetListRowNum(MPA1501Context model)
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private static string GetSelectedTicket(MPA1501Context model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static Func<AinokazePaymentInfo, MPA1501ReportDataRow> ToMPA1501ReportDataRow() =>
            t =>
                new MPA1501ReportDataRow()
                {
                    UserId = t.UserId,
