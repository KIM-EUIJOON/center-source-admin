...
{
    public class MPA1502Controller : Controller
    {    
        //ファイル出力関連
        ...
        // セッションキー
        private const string SESSION_SEARCH_MPA1502 = "SESSION_SEARCH_MPA1502";

        // 一覧ページ単位の表示件数
        private const int LISTMAXROW = 10;

        // ルートパス
        private const string CONTENTROOT = "~/Views/MPA1502";

        /// <summary>
        /// 決済データ画面(初期, ページ遷移)
        /// </summary>
        /// <returns>ログイン画面</returns>
        [SessionCheck(WindowName = "乗車券利用実績画面(あいの風とやま様)")]
        public ActionResult Index(string page)
        {
            ...
            if (string.IsNullOrEmpty(page))
            {
                ...
                return View(new MPA1502Context());
            }

            //セッション情報の取得
            Dictionary<string, string> searchKey = new Dictionary<string, string>();
            searchKey = (Dictionary<string, string>)Session[SESSION_SEARCH_MPA1502];
            //検索条件：開始日時
            ...
            if (usageList.Any())
            {
                info.UsageReportData = new MPA1502ReportData()
                {
                    ListMaxCount = int.Parse(maxListCount),
                    ListNum = ListNum,
                    ReportList = usageList.Select(ToMPA1502ReportDataRow()).ToList(),
                };
            }
            else
            {
        ...
        [HttpPost]
        [SessionCheck(WindowName = "乗車券利用実績画面(あいの風とやま様)")]
        public ActionResult Index(MPA1502Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                Session.Add(SESSION_SEARCH_MPA1502, searchKey);
        ...
        [HttpPost]
        [SessionCheck(WindowName = "乗車券利用実績画面(あいの風とやま様)")]
        public ActionResult OutPutCSV(MPA1502Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                using (var sw = new StreamWriter(ms, System.Text.Encoding.GetEncoding("shift_jis")))
                {
                    ...
                    //ファイル名を「AinokazeToyama_Usage_検索開始日(yyyyMMdd)-終了日(yyyyMMdd)_作成日(yyyyMMdd)」で出力
                    return File(ms.ToArray(), FILE_CONTENTTYPE, "AinokazeToyama_Usage_" + targetDateStart.ToString("yyyyMMdd") + "-" + targetDateLast.ToString("yyyyMMdd") + "_" + NowDate.ToString("yyyyMMdd") + FILE_EXTENSION);
                }
            }
            // 入力エラー
            catch(InvalidInputException iie)
        ...
        /// </summary>
        /// <returns></returns>
        private static AinokazeTicketSaleRepository GetTicketListRespository()
        {
            return new AinokazeTicketSaleRepository();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private static AinokazeTicketUsageRepository GetTicketUsageRepository()
        {
            return new AinokazeTicketUsageRepository();
        }
        ...
        private static void IsValidInput(MPA1502Context model)
        {
        ...
        private static (int Begin, int End) GetListRowNum(MPA1502Context model)
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private static string GetSelectedTicket(MPA1502Context model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static Func<AinokazeUsageInsuranceInfo, MPA1502ReportDataRow> ToMPA1502ReportDataRow() =>
            t =>
                new MPA1502ReportDataRow()
                {
                    UserId = t.UserId,
        ...
        /// <returns></returns>
        private static List<SelectListItem> CreatePDLItemsTicketList(IEnumerable<AinokazePaymentInfo> infos, string selected = null)
