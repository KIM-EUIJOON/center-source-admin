...
using AppSigmaAdmin.Repository.Nishitetsu;
using AppSigmaAdmin.ResponseData;

namespace AppSigmaAdmin.Controllers
{
    public class MPA1402Controller : Controller
    {
        //ファイル出力関連
        ...
        // セッションキー
        private const string SESSION_SEARCH_MPA1402 = "SESSION_SEARCH_MPA1402";

        // 一覧ページ単位の表示件数
        private const int LISTMAXROW = 10;

        // ルートパス
        private const string CONTENTROOT = "~/Views/MPA1402";

        /// <summary>
        /// 決済データ画面(初期, ページ遷移)
        /// </summary>
        /// <returns>ログイン画面</returns>
        [SessionCheck(WindowName = "乗車券利用実績画面(西鉄様)")]
        public ActionResult Index(string page)
        {
            ...
            if (string.IsNullOrEmpty(page))
            {
                ...
                return View(new MPA1402Context());
            }

            //セッション情報の取得
            Dictionary<string, string> searchKey = new Dictionary<string, string>();
            searchKey = (Dictionary<string, string>)Session[SESSION_SEARCH_MPA1402];
            //検索条件：開始日時
            ...
            if (usageList.Any())
            {
                info.UsageReportData = new MPA1402ReportData()
                {
                    ListMaxCount = int.Parse(maxListCount),
                    ListNum = ListNum,
                    ReportList = usageList.Select(ToMPA1402ReportDataRow()).ToList(),
                };
            }
            else
            {
        ...
        [HttpPost]
        [SessionCheck(WindowName = "乗車券利用実績画面(西鉄様)")]
        public ActionResult Index(MPA1402Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                Session.Add(SESSION_SEARCH_MPA1402, searchKey);
        ...
        [HttpPost]
        [SessionCheck(WindowName = "乗車券利用実績画面(西鉄様)")]
        public ActionResult OutPutCSV(MPA1402Context model)
        {
            try
            {
                ViewData["message"] = "";
                ...
                using (var sw = new StreamWriter(ms, System.Text.Encoding.GetEncoding("shift_jis")))
                {
                    ...
                    //ファイル名を「Nishitetsu_Usage_検索開始日(yyyyMMdd)-終了日(yyyyMMdd)_作成日(yyyyMMdd)」で出力
                    return File(ms.ToArray(), FILE_CONTENTTYPE, "Nishitetsu_Usage_" + targetDateStart.ToString("yyyyMMdd") + "-" + targetDateLast.ToString("yyyyMMdd") + "_" + NowDate.ToString("yyyyMMdd") + FILE_EXTENSION);
                }
            }
            // 入力エラー
            catch(InvalidInputException iie)
        ...
        /// </summary>
        /// <returns></returns>
        private static NishitetsuTicketSaleRepository GetTicketListRespository()
        {
            return new NishitetsuTicketSaleRepository();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private static NishitetsuTicketUsageRepository GetTicketUsageRepository()
        {
            return new NishitetsuTicketUsageRepository();
        }
        ...
        private static void IsValidInput(MPA1402Context model)
        {
        ...
        private static (int Begin, int End) GetListRowNum(MPA1402Context model)
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private static string GetSelectedTicket(MPA1402Context model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private static Func<NishitetsuUsageInfo, MPA1402ReportDataRow> ToMPA1402ReportDataRow() =>
            t =>
                new MPA1402ReportDataRow()
                {
                    UserId = t.UserId,
        ...
        /// <returns></returns>
        private static List<SelectListItem> CreatePDLItemsTicketList(IEnumerable<NishitetsuPaymentInfo> infos, string selected = null)
